...
$query = "SELECT DISTINCT varietal FROM bottle ";
$query .= "WHERE quantity>0 AND beginconsume=$year ORDER BY varietal";
...
$totquery = "SELECT * FROM bottle WHERE beginconsume=$year";
...
while ($row = @mysql_fetch_row($result)) {
   ...
   $newquery = "SELECT * FROM bottle WHERE varietal=\"$row[0]\"";
   $newquery .= " AND beginconsume=$year";
...
$graph = new Graph(700,360);
$graph->SetScale("textlin");

$bplot = new BarPlot($percent);
$bplot->SetFillColor("slategray2");

$graph->img->SetMargin(35,30,30,130);
$graph->title->Set("% of Bottles Entering Maturity in $year per Varietal");
