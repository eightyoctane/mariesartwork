...
if (! isset($_POST['submitbutton'])){
    ...
    //$lf->logMessage("profileEdit.php submit button not set this must be a\nstandalone request select this user\n");
	
	//echo "profileEdit.php: software error (please contact $sysadminmail <br />";
	$uname = $_SESSION['UserName'];
	$query = "SELECT rowID, name, username, email, add_apt, add_street, add_city, add_state, add_zip, add_country, u_type, u_priv, phone FROM user WHERE username='".$uname."'";	
	try {
		//$mysqldb->query("SELECT rowID, name, username, email, add_apt, add_street, add_city, add_state, add_zip, add_country, u_type, u_priv, phone FROM user WHERE username='eightyoctane'");
		$mysqldb->query($query);
	} catch (Exception $e) {
		$lf->logMessage($e);
		$lf->setErrorStatus($e);
