...
*/
	...
	public function sessionSetup($usertype, $minpriv, $prevpage) {
	
	if(isset($_SESSION['UserName'])) {
			
		if($_SESSION['u_type'] != $usertype) {
			
			$this->lf->logMessage("User: ".$_SESSION['UserName']." has incorrect user type");
			...
			$this->lf->logMessage("User:".$_SESSION['UserName']." access requires higher privilege\n");
		...
		$this->lf->logMessage("sessionSetup: Not in session but should be\n");
		session_unset();
		session_destroy();
		header( 'Location: '.$this->getRootLocation().'login/login.php');
	}
}
