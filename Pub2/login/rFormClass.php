...
class Form {
	...
	public function setTextEditorArea($rows, $cols, $name, $value, $toolbar, $errortext) {
	
		//$editinstance .= '<textarea id="'.$name.'" name="'.$name.'" rows="'.$rows.'" cols="'.$columns.'">'.$value.'</textarea> ';
		$editinstance .= '<textarea id="'.$name.'" name="'.$name.'">'.$value.'</textarea> ';
		//echo "<p>*******toolbar: ".$toolbar."</p>";
		$editinstance .= '<script type="text/javascript">CKEDITOR.replace( \''.$name.'\', {toolbar : \''.$toolbar.'\'});</script>';
		//$editinstance .= '<script type="text/javascript">CKEDITOR.replace( \''.$name.'\');</script>';
		
		$this->editinstance[$name] = array(	"rows" => $rows,
										"columns" => $columns,
										"html" => $editinstance
									);
	...
	public function displayTextEditorArea($name) {
		return($this->editinstance[$name]['html']);	
	}

	public function getTextEditorArea($rows, $cols, $name, $value, $toolbar, $error) {
		if ($_POST[$name] == NULL) {
			$this->setTextEditorArea($rows,$cols,$name, $value, $toolbar, $error);
			
			///$this->lf->logMessage("no name entry\n");
			return(NULL);
		} else {
			$this->setTextEditorArea($rows, $cols, $name, $_POST[$name], $toolbar, " ");
	...
	public function returnHyperlink($id,$target,$disp) {
		if( $disp === 'new') {
			return('<a target="_blank" href="'.$target.'">'.$id.'</a>');
		} else {
	...
	public function setHyperlinkPerLine($id, $target, $disp) {
		if( $disp === 'new') {
			$this->hyperlink[$id] .= '<p><a target="_blank" href="'.$target.'">'.$id.'</a></p>';
		} else {
			$this->hyperlink[$id] .= '<p><a href="'.$target.'">'.$id.'</a></p>';
		}
	}

	public function returnHyperlinkPerLine($name, $target, $disp, &$hyperlink) {
		if( $disp === 'new') {
			$hyperlink .= '<p><a target="_blank" href="'.$target.'">'.$name.'</a></p>';
		} else {
			$hyperlink .= '<p><a href="'.$target.'">'.$name.'</a></p>';
		}
	}
	...
	public function getCBList($cblistname, &$valuearray) {
	
		$index = 0;
		while ($valuearray[$index]['name']) {
			$locator = $cblistname."-".$valuearray[$index]['id'];
			//echo "<p>*****getCBList: locator: ".$locator."</p>";
			$valuearray[$index]['sense'] = $this->getCheckBox($locator);
			$this->cblist[$index][$cblistname]['name'] = $valuearray[$index]['name'];
			$this->cblist[$index][$cblistname]['htmlocator'] = $locator;

			//echo "<p>*****getCBList: name: ".$valuearray[$index]['name']." id: ".$valuearray[$index]['id']." sense: ".$valuearray[$index]['sense']."</p>";
			$index++;
		}
		
	}
	
	public function displayCBList($cblistname) {
	
		$index = 0;
		while ($this->cblist[$index][$cblistname]['name']) {
			//echo "<p> **** indicator: ".$this->cblist[$index][$cblistname]['htmlocator']."</p>";
			$list .= "<p>".$this->displayCheckbox($this->cblist[$index][$cblistname]['htmlocator'])." ".$this->cblist[$index][$cblistname]['name']."</p>";
			//echo "<p> ".$this->displayCheckbox($this->cblist[$index][$cblistname]['htmlocator'])."</p>";
			$index++;
		}
		return ($list);	
	}

	public function initializeCheckBoxList($cblistname, &$valuearray) {
		$index=0;
		while($valuearray[$index]) {
			$locator = $cblistname."-".$valuearray[$index]['id'];
			$this->setCheckbox($locator, $valuearray[$index][$cblistname]['sense']);
			$this->cblist[$index][$cblistname]['name'] = $valuearray[$index]['name'];
			$this->cblist[$index][$cblistname]['htmlocator'] = $locator;
			//echo "<p> *****name: ".$this->cblist[$index][$cblistname]['name']." locator: ".$this->cblist[$index][$cblistname]['htmlocator']."</p>";
			$index++;	
		}
	}
	...
	public function setCheckbox($name, $sense) {
		if( $sense == 1) {
			...
			//echo "<p> setting checkbox: ".$this->checkbox[$name]."</p>";
		} else {
	...
	public function getTextBox($col, $name,$error) {
		if ($_POST[$name] == NULL) {
			...
			//$this->setTextBox($this->textbox[$name]['columns'],$name, mysql_real_escape_string ($_POST[$name]), " ");
			$this->setTextBox($col,$name, mysql_real_escape_string ($_POST[$name]), " ");
	...
	public function setSimpleTextLine($name, $value) {
		$this->stext[$name] .= '<p><font size="4" color="#FF0000"><span style="background-color: #FFFF00">'.$value.'</span></font></p>';
	}
	...
	public function getTextArea($rows, $cols, $name,$error) {
		if ($_POST[$name] == NULL) {
			...
			$this->setTextArea($rows, $cols, $name, mysql_real_escape_string ($_POST[$name]), " ");
...
class iForm extends Form {
	...
	public function igetTextBox($name, $id, $error) {
		return( parent::getTextBox(1, $this->iencodename($name, $id), $error) );
	}
	...
	public function inputComplete(&$retarr) {
		...
		if (!($retarr['vdeadline'] = $this->getTextBox(4, 'vdeadline', "must be set"))) $refresh = TRUE;
...
class pjForm extends Form {
	...
	public function initializeProjectForm() {
	
		include($_SERVER['DOCUMENT_ROOT']."/homedatapub2.php");
		include($rootlocation."login/VisitorDataClass.php");
		$vd = new VisitorData($this->lf);
		
		$this->setTextBox(30, "pjname", "", "* can be multiple words");
		$this->setTextBox(15, "descname", "", "*short single word name here");
		$this->setTextArea(2, 29, "shortdesc", "", "*");
		$this->setTextArea(14, 29, "abstract", "", "*");
		
		$uprivarr[0]=$upriv['anyowner'];
		$uprivarr[1]=$upriv['committee'];
		$uprivarr[2]=$upriv['commchair'];
		$uprivarr[3]=$upriv['board'];
		$uprivarr[4]=$upriv['boarpres'];
		$vd->getIdNamePairs($utype['resident'], &$uprivarr, &$valuearray);
		
		$index=0;
		while($valuearray[$index]) {
			$valuearray[$index]['auth']['sense']=0;
			$index++;		
		}
		
		$this->initializeCheckBoxList('auth', &$valuearray);
	...
	public function inputComplete(&$retarr) {
		...
		$vd->getIdNamePairs($utype['resident'], &$uprivarr, &$idarray);
		//echo "<p>*****inputComplete getting checkbox list idarray</p>";
		//print_r($idarray);
		$this->getCBList('auth',&$idarray);
		//echo "<p>*****getCBList: idarray populated with sense</p>";
		//print_r($idarray);
		
		$retarr['auth'] = serialize($idarray);
		
		//$temparr = unserialize($retarr['auth']);
		//echo "<p>****** unserialize test:</p>";
		//if( !is_array($temparr) ) echo "<p>oops not an array</p>";
		//print_r($temparr);
					
		$refresh = FALSE;
			
		if ( !($retarr['pjname'] = $this->getTextBox(30, 'pjname', "must be set")) ) $refresh = TRUE;
		
		if (!($retarr['descname'] = $this->getTextBox(15, 'descname', "must be set"))) $refresh = TRUE;

		if (!($retarr['shortdesc'] = $this->getTextArea(2, 29, 'shortdesc', "must be set"))) $refresh = TRUE;
	...
	public function initializeCommentForm() {
	
		include($_SERVER['DOCUMENT_ROOT']."/homedatapub2.php");

	
		//echo "<p>*******initializeCommentForm: Not written yet</p>";
			
		$this->setProjectNameSelectionBox("projects", 0);
		
		$this->setTextEditorArea(10, 40, 'comment', "", $cktoolbar, "");	
	}
	
	public function cinputComplete($sp, &$valuearray) {
	

		include($_SERVER['DOCUMENT_ROOT']."/homedatapub2.php");
		require_once($rootlocation."login/Projects/project.php");		
		
		$userid = $sp->getUserID();
		$refresh = FALSE;
		$prj = new Project($this->lf);
		
		if ( ($pjid = $this->getProjectIDFromSelectionBox("projects")) == 0 ) {
			$this->getTextEditorArea(10,40, 'comment', "", $cktoolbar, "");
			return(FALSE);	
		}
		
		
		//echo "<p>cinputComplete/rFormClass.php: pjid: ".$pjid."</p>";
		
		//check authorization
		if( !$prj->isAuthorized($sp->getUserID(), $pjid) ) {
			$emsg = "Sorry, you're not authorized for this project. Please check with the Committee chairman.";
			$sp->setErrorStatusBarMessage($emsg);
			$this->setSimpleErrorText('projects', $emsg);

			$refresh = TRUE;
			echo "<p>cinputComplete/rFormClass.php: not authorized</p>";
			return(FALSE);
		}
		
		$prow=$prj->getProject($pjid);
		$projdir = $prow['filesdir'];

		$this->setSubmit('submitbutton','Upload');
		
		//collect comments for this project
		if( ($valuearray['comment'] = $this->getTextEditorArea(10,40, 'comment', "", $cktoolbar, "")) ) {
			//this user is entering a new comment
			//put the comment in the database right now
			$prj->addComment($pjid, &$valuearray);
			//now reset the text area
			$this->setTextEditorArea(10,40, 'comment', "", $cktoolbar, "");

		}
		
		while( ($row=$prj->nextComment($pjid)) ) {
			if( isset($_POST['delete-'.$row->id]) ) {
				if( ($sp->getUserID() == $row->submittedbyid) || ($sp->getUserPriv() == $upriv['webadmin']) ) {
					$prj->deleteComment($pjid, $row->id);
				} else {
					$this->setSimpleErrorText('deletecomment', "Sorry, you can only delete comments which you created");
				}
			}
		}
		
		$this->setCommentList($pjid, $prj);
		
		$targetdir = $rootlocation."login/ProjectFiles/".$projdir."/"; 
		$fullfilename = $_FILES['uploaded']['name'];
		//echo "<p>*****fullfilename: ".$fullfilename."</p>";
		$filename = basename( $_FILES['uploaded']['name']) ;
		$target = $targetdir . basename( $_FILES['uploaded']['name']) ; 
		...
		if ($uploaded_size > $maxprojfilesize) { 
			echo "Sorry, your file is too large.<br>";
			$msg = "Sorry, your file is too large ".$maxprojfilesizestring.", please try a smaller file. "; 
			$ok=0; 
		} else
		...
		$this->setSimpleErrorText('uploaded', $msg);
		if( !$ok ) {			
			$refresh = TRUE;
		}

		//echo "<p>**************setting up hyperlinks</p";


		//list project file hyperlinks
		
		$this->setFileList($targetdir, $homelocationpart.$homelocationsub.'login/ProjectFiles/'.$projdir.'/');
	...
	public function setCommentList($pjid, $prj) {
		include($_SERVER['DOCUMENT_ROOT']."/homedatapub2.php");
		require_once($rootlocation."login/TableClass.php");		
		$t = new Table();
		$t->setBoarderSize(1);
		$t->addRow($t->RCQ("Comment").$t->RCQ("Submitter").$t->RCQ("Submitted Date").$t->RCQ("Edit").$t->RCQ("Delete"));
		while( ($row = $prj->nextComment($pjid)) ) {
			$editcheckbox = $t->RCQ('<input type="checkbox" name="edit-'.$row->id.'" value=0>');
			$deletecheckbox = $t->RCQ('<input type="checkbox" name="delete-'.$row->id.'" value=0>');
			$t->addRow( $t->RCQ($row->comment).$t->RCQ($row->submittedbynm).$t->RCQ(date('r', $row->submittedtime)).$editcheckbox.$deletecheckbox);
		}
		$this->ctext = $t->renderTableText();
		//echo "<p>*****</p>".$this->ctext."<br>";
	}
	...
	public function setHyperlinklist($dir) {
		include($_SERVER['DOCUMENT_ROOT']."/homedatapub2.php");
		include($rootlocation."login/TableClass.php");		
		$t = new Table();		
		//echo "<p>******setHyperlinklist: dir: ".$dir."</p>";
		// Open a known directory, and proceed to read its contents
		if (is_dir($dir)) {
			//echo "<p>******* is a dir! ".$dir."</p>";
		    if ($dh = opendir($dir)) {
		    	//echo "<p>****** im in the directory</p>";
		        while (($file = readdir($dh)) !== false) {
		        	if ($file != "." && $file != "..") {
			            echo "filename: $file : filetype: " . filetype($dir . $file) . "\n";
			            $this->returnHyperlinkPerLine($file, $homelocationpart.$homelocationsub.'login/ProjectFiles/'.$file, 'new', &$this->hyperlinklist);
		        	}
		        }
		        closedir($dh);
		    }
	...
	public function setFileList($dir, $linkdir) {
		include($_SERVER['DOCUMENT_ROOT']."/homedatapub2.php");
		require_once($rootlocation."login/TableClass.php");		
		$t = new Table();
		$t->setBoarderSize(1);		
		//echo "<p>******setFileList: dir: ".$dir."</p>";
		// Open a known directory, and proceed to read its contents
		if (is_dir($dir)) {
			//echo "<p>******* is a dir! ".$dir."</p>";
		    if ($dh = opendir($dir)) {
		        ...
		        while (($file = readdir($dh)) !== false) {
		        	if ($file != "." && $file != "..") {
			            ...
			            $t->addRow( 
			            $t->RCQ( $this->returnHyperlink($file, $linkdir.$file, 'new', &$this->hyperlinklist) ).
			            $t->RCQ( date('r', filemtime($dir.$file)) )
			            );
			            //$filecode .= $t->addRow($t->RCQ("xxxxxxxxxxxxxxxxxx".$file));
			            $this->filelist .= $filecode;
			            //echo "<p>**********file html: ".$filecode."</p>";
		        	}
		        }
		        closedir($dh);
		        $this->filelist = $t->renderTableText();
		    }
		}
	}	
	...
	public function displayHyperlinkList() {
		return($this->hyperlinklist);		
	}

	public function ceditInputComplete() {
	
		echo "<p>*********ceditInputComplete() not written yet</p>";	
	}

	public function setProjectNameSelectionBox($retname, $selectionindex) {
		// get the appropriate project names from the database
	
		include($_SERVER['DOCUMENT_ROOT']."/homedatapub2.php");
		include_once($rootlocation."login/Projects/project.php");

		//echo "<p>*********setProjectNameSelectionBox selectionindex: ".$selectionindex."</p>";


		$in = new Project($this->lf);
		$uname[0] = array(0, "must be set");
		$unamekey = 1;
		while( ($row = $in->nextProject() ) ) {
			//echo "<p> set name selection box: ".$row->iname."</p>";
			$uname[$unamekey] = array($unamekey, $row->pjname);
			$encoded = "pjid".$unamekey;
			$this->setHiddenValue('"'.$encoded.'"', $row->pjid);
			//$lf->logMessage("name: ".$uname[$unamekey]."\n");
			$unamekey = $unamekey +1;
		}
	...
	public function getProjectIDFromSelectionBox($name) {
	
		//include($_SERVER['DOCUMENT_ROOT']."/homedatapub2.php");
		//include_once($rootlocation."login/Initiatives/initiative.php");
		
		// here is the index provided by the user	
		$index = $this->getNameListSelectedIndex($name);
		//echo "<p>****** getProjectIDFromSelecteonBox select index: ".$index."</p>";
		if( $index == 0 ) 	{
			$this->setProjectNameSelectionBox($name, 0);
			$this->setSimpleErrorText("$name", "must be set");
			return(NULL);
		} else {
			$this->setProjectNameSelectionBox($name, $index);
			$this->setSimpleErrorText("$name", "");
		}

		$pjid = $this->getHiddenValue("pjid".$index);
		//echo "<p>****** getProjectIDFromSelecteonBox pjid: ".$pjid."</p>";
		...
		if( $iid != 0 ) {
			...
			while($row = $prop->nextProposal()) {	
				if( $iid == $row->iid && $row->pstatus == 'open' ) {
					...
					//echo "<p>******returnProposalDetail found a proposal: ".$row->pname."</p>";
