...
class Form {
	...
	public function setTextEditorArea($rows, $cols, $name, $value, $errortext) {
	...
	public function getTextEditorArea($rows, $cols, $name, $error) {
		if ($_POST[$name] == NULL) {
			...
			$this->setTextEditorArea($rows, $cols, $name, $_POST[$name], " ");
...
class pjForm extends Form {
	...
	public function getProjectIDFromSelectionBox() {
	
		include($_SERVER['DOCUMENT_ROOT']."/homedatapub2.php");
		include_once($rootlocation."login/Initiatives/initiative.php");
		
		// here is the index provided by the user	
		$index = $this->getNameListSelectedIndex('projects');
