			...
			if($row->istatus === "open") {
				...
				if( ($iid != $row->iid) ) {
					...
					if( $vote['yes'] && $vote['no']) {
						...
						$if->isetSimpleErrorText("voteerror", $iid, "we didn't get your vote please select yes or no");
						$if->isetCheckbox("yesvote",0);
						$if->isetCheckbox("novote",0);
						$vformtext = $this->renderFormText();
					} else {
...
class pForm extends Form {
	...
	public function inputComplete(&$retarr) {
		...
		if (!($retarr['pname'] = $this->getTextBox('pname', "must be set"))) {
			$refresh = TRUE;
		} else {
			// NOTE need addnl processing here to insure legal name
			$retarr['pname'] = $this->getTextBox('pname', " ");
		}
		...
		if (!($retarr['duration'] = $this->getTextBox('duration', "must be set"))) {
			$refresh = TRUE;
		} else {
