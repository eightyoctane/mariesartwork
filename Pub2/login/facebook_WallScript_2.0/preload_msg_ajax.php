 ...
 <?php
		...
		$selectstring = "select messages.u_id as mu_id, messages.msg_id as msg_id, messages.message as message, user.name as name from user, messages where messages.u_id = user.rowID  order by msg_id desc";
		...
		while($row=mysql_fetch_array($result)) {
			...
			$msg=toLink($msg);
			$name=$row['name'];
			$lf->logMessage("preload_msg: Found message for user ".$name." id: ".$id."\n");
		...
		?>
			...
			<span class="delete_button"><a href="#" id="<?php echo $id; ?>" muid="<?php echo $mu_id?>" class="delete_update">X</a></span>
...
<?php
			$selectstring = "select comments.msg_id_fk as msg_id_fk, comments.com_id as com_id, comments.comment as comment, user.name as name from user, comments where comments.u_id = user.rowID and comments.msg_id_fk = $id order by com_id desc";
			...
			while($rowc=mysql_fetch_array($resultc)) {
				...
				$msg=toLink($comment);
				$name=$rowc['name'];
				$lf->logMessage("preload_msg_ajax.php: Found comment for message id: ".$msg_id." comment id: ".$idc."\n");
