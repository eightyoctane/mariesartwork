...
class iForm extends Form {
	...
	public function isetCheckbox($name, $id, $sense) {
		parent::setCheckbox($name."-".$id, $sense);
	}
	
	
	public function igetCheckBox($name) {
		if (!isset($_POST[$name])) {
			//echo "**************".$name." is zero";
	...
	public function icheckboxValue($name) {
		if (!isset($_POST[$name])) {
			//echo "**************".$name." is zero";
			parent::setCheckbox($name, 0);
			$retval = FALSE;
...
*/
	...
	public function returnInitiativeDetailTable($vid, $iid, $if) {
			...
			if($row->istatus === "open") {
				...
				if( ($iid != $row->iid) ) {
					...
					if( !$ballet->getBalletInfo($vrow->vtablename, $vid, &$votet) ) {
						...
						print_r($votet);
						$this->isetCheckbox("yesvote", $this->iid, $this->setCheckbox("yesvote", $iid, $votet['yes']), $row->iid);		
						$this->isetCheckbox("novote", $this->iid, $if->setCheckbox("novote", $iid, $votet['no']), $row->iid);
						$this->isetSimpleErrorText("voteerror", $row->id, "");
					...
					if( $vote['yes'] && $vote['no']) {
						...
						$this->setCheckbox("yesvote", $iid, 0);
						$this->setCheckbox("novote", $iid, 0);
						$vformtext = $this->renderFormText();
					} else if( !$vote['yes'] && !$vote['no'] ) {
						echo "<p>neither box checked</p>";
						$if->setSimpleErrorText("voteerror", $iid, "we didn't get your vote please select yes or no");
