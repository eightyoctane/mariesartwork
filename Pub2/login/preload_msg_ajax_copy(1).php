 ...
 <?php
		...
		$selectstring = "select messages.u_id as mu_id, messages.msg_id as msg_id, messages.message as message, user.name as name from user, messages where messages.u_id = user.rowID  order by msg_id desc limit $maxentries";
		$result=mysql_query($selectstring);
	
		//$indexptr = 0;
		//while($row=mysql_fetch_array($result) && $indexptr < 2) {
		while($row=mysql_fetch_array($result)) {

			$indexptr = $indexpter + 1;
			$mu_id=$row['mu_id'];
			$id=$row['msg_id'];
			$msg=$row['message'];
			$msg=toLink($msg);
			$name=$row['name'];
			$lf->logMessage("preload_msg: Found message for user ".$name." userid: ".$mu_id." id: ".$id."\n");
...
<?php
			$selectstring = "select comments.u_id as cmu_id, comments.msg_id_fk as msg_id_fk, comments.com_id as com_id, comments.comment as comment, user.name as name from user, comments where comments.u_id = user.rowID and comments.msg_id_fk = $id order by com_id desc";
			...
			while($rowc=mysql_fetch_array($resultc)) {
				...
				$msg=toLink($comment);
				$name=$rowc['name'];
				$lf->logMessage("preload_msg_ajax.php: Found comment for userid: ".$cmu_id." message id: ".$msg_id." comment id: ".$idc."\n");
