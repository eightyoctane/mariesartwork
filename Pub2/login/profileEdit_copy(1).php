...
$sp = new sessionPageBrowser($rootlocation.$regionheader,$upriv['anyowner'],$homelandingurlfull[$utype['webadmin']]);
...
if (! isset($_POST['submitbutton'])){
	...
	$of = new rForm($rootlocation."login/rFormABasic.html");
	$of->setWebsite($nameid);
	$of->setFormName("User Profile Editor - Password will be changed only if a new value is entered");
	$of->setEditCallBackPgm("/login/profileEdit.php");
	$of->setSuccessCallBackPgm("/login/ListAndSelectUsers.php");
	...
	if ($_POST['uname'] == NULL) {
		...
		$_POST['country'], $_POST['phone'],$tboxhtml,$pboxhtml,$tmsg,$pmsg, $of->getCheckBox());
	...
	//echo "<p>Good, profile is in the database</p>";
		...
		if (!CheckNameValidity($_POST['name'], $msg)) {
			...
			$of->renderRformText($msg, NULL, NULL, NULL, NULL, $_POST['uname'], 
			$_POST['email'], $_POST['apt'], $_POST['street'], $_POST['city'], $_POST['state'],$_POST['zip'], 
			$_POST['country'], $_POST['phone'],$tboxhtml,$pboxhtml,$tmsg,$pmsg, $of->getCheckBox());
