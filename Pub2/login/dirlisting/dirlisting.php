...
function dirlisting($lf) {
		...
		global $displayindex;
		global $hide;
		global $forcedownloads;
		global $showdirs;
		global $showthumbnails;
		global $startdir;	
		...
		$lf->logMessage('$startdir: '.$startdir."\n");
		
		if($leadon=='.') $leadon = '';
		
		//add a slash if we have a non blank leadon with no slash
		if((substr($leadon, -1, 1)!='/') && $leadon!='') $leadon = $leadon . '/';
		
		$startdir = $leadon;
		
		$lf->logMessage("This server: ".$_SERVER['PHP_SELF']."\n");
		$lf->logMessage("This document root: ".$_SERVER['DOCUMENT_ROOT']."\n");
		$lf->logMessage("Directory Listing of ".dirname($_SERVER['PHP_SELF']).'/'.$leadon."\n");
		$lf->logMessage('$startdir: '.$startdir."\n");			
		...
		if($_GET['dir']) {
			...
			$lf->logMessage("GET dir: ".$_GET['dir']."\n");
		...
		if($_GET['download'] && $forcedownloads) {
			...
			$lf->logMessage("$_GET download and forcedownloads\n");
		...
		if($allowuploads && $_FILES['file']) {
			...
			$lf->logMessage("allowuploads and $_FILES\n");
