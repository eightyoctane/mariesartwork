...
class pjForm extends Form {
	...
	public function cinputComplete($sp, &$valuearray) {
		...
		//echo "<p>*******cinputComplete: Not written yet</p>";	
		...
		echo "<p>commentfromproject.php: pjid: ".$pjid."</p>";
		
		//check authorization
		if( !$prj->isAuthorized($sp->getUserID(), $pjid) ) {
			$sp->setErrorStatusBarMessage("Sorry, you're not authorized for this project. Please check with the Committee chairman.");
			$refresh = TRUE;
		};
		
		//collect comments for this project
		if( !$this->getTextEditorArea(10,40, 'comment', "", $cktoolbar, "must be set") ) $refresh = TRUE;
